...
def premier_one_service(number: int):
	driver.get('https://premier.one/')
	driver.implicitly_wait(5)
	btn_preload_form = driver.find_element(By.CLASS_NAME, 'user-avatar__name-label').click()
...
def lenta_service(number: int):
	driver.get('https://novosibirsk.online.lenta.com/')
	driver.implicitly_wait(5)
	preload_form = driver.find_element(By.CLASS_NAME, 'header-login-btn').click()
	phone_form = driver.find_element(By.ID, 'profilePhoneInput')
	phone_form.send_keys(number)
	phone_form.send_keys(Keys.RETURN)
